...
describe("DiversifyPeersBySubnetTask", function () {
  ...
  it("should search all subnets, network call throw exception", async () => {
    const peerId = await PeerId.create();
    networkStub.getPeers.returns([peerId]);
    reqRespStub.metadata.throws(new Error("error from unit test"));
    await task.run();
  ...
  it("should search all subnets, no peer", async () => {
    ...
    reqRespStub.metadata.throws(new Error("error from test"));
    await task.run();
    expect(networkStub.searchSubnetPeers.callCount).to.be.equal(64);
  });
  ...
  it("should search 61 subnets", async () => {
    ...
    networkStub.getPeers.returns([peerId, peerId2]);
    const attNets = Array(64).fill(false);
    attNets[0] = true;
    attNets[1] = true;
    reqRespStub.metadata.onFirstCall().resolves({
      attnets: attNets,
      seqNumber: BigInt(1),
    });
    ...
    attNets2[1] = true;
    attNets2[2] = true;
    reqRespStub.metadata.onSecondCall().resolves({
      attnets: attNets2,
      seqNumber: BigInt(1),
    });
    await task.run();
